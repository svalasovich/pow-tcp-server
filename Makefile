...
.PHONY: lint
lint:
	docker run --rm -v ${PWD}:/app -w /app golangci/golangci-lint:v1.59.1 golangci-lint run --config=./.github/linters/.golangci.yml --timeout=5m

#? docker: Build docker image
.PHONY: docker
docker:
	docker build -t svalasovich/golang-template -f ./build/package/Dockerfile .

#? mock: Generate mock files
.PHONY: mock
mock:
	docker run --rm -v "$PWD":/src -w /src vektra/mockery:v2.44.1 --all --keeptree --case=underscore --output=/src/test/mock
